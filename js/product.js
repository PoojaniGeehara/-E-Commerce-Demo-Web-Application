...
function renderProductCard(product) {
  ...
  return `
    <article class="product-card card" data-product-id="${product.id}">
      <a href="product-details.html?id=${product.id}" class="product-card-image">
        <img src="${product.image}" alt="${product.name}" onerror="this.parentElement.textContent='${CATEGORY_ICONS[product.category] || "\u{1F6CD}"}'">
      </a>
...
function renderCategoryGrid() {
  ...
  container.innerHTML = CATEGORIES.map(cat => `
    <div class="category-card card">
      ...
      <h3>${cat}</h3>
      <a href="pages/products.html?category=${encodeURIComponent(cat)}" class="btn btn-outline btn-sm">View Products</a>
    </div>
  `).join("");
}
  ...
  function applyFilters() {
    ...
    if (priceRange && priceRange !== "all") {
      ...
      results = results.filter(p => {
        ...
        if (max) return finalPrice >= min && finalPrice <= max;
...
function initProductDetailsPage() {
  ...
  container.innerHTML = `
    <div class="product-details-image">
      <img src="${product.image}" alt="${product.name}" onerror="this.parentElement.textContent='${CATEGORY_ICONS[product.category] || "\u{1F6CD}"}'; this.parentElement.style.fontSize='4rem';">
    </div>
